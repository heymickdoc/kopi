...
namespace Kopi.Core.Services.Strategies;

public class StandardTableDataStrategy(
    SourceDbModel sourceDbData,
    DataGeneratorService generatorService) : ITableDataStrategy
{
    ...
    public async Task<List<RowData>> GenerateTableDataAsync(
        TableModel table,
        int maxRowCount,
        List<TargetDataModel> existingData)
    {
        // --- THIS IS THE METHOD FROM YOUR OLD FILE ---
        // I have adapted it to use the passed-in 'existingData' 
        // instead of the class-level field 'existingData'.
        ...
        int smallestBatch = maxRowCount;

        // ... [Copy/Paste your Passes 1, 2, 2.5, 3, 4, 5 here] ...
        // --- PASS 1: Composite Foreign Keys ---
        var compositeFks = FindCompositeForeignKeys(table, sourceDbData.Relationships);
        foreach (var cfk in compositeFks)
        {
            ...
            var batch = GenerateCompositeFkBatch(cfk, smallestBatch, existingData);
        ...
        if (primaryKey != null && !primaryKey.PrimaryKeyColumns.Any(processedColumns.Contains))
        {
            ...
            var batch = GenerateCompositePkBatch(table, primaryKey, sourceDbData.Relationships, smallestBatch,
                existingData);
        ...
        foreach (var uqIndex in uniqueCompositeIndexes)
        {
            ...
            var batch = GenerateCompositeUniqueIndexBatch(table, indexColumnNames, smallestBatch, existingData);
        ...
        {
            var batch = GenerateSingleUniqueBatch(column, table, sourceDbData.Relationships, smallestBatch,
                existingData);
        ...
        {
            var batch = GenerateRegularBatch(column, table, sourceDbData.Relationships, smallestBatch, existingData);
            generatedBatches[column.ColumnName] = batch;
            processedColumns.Add(column.ColumnName);
        }
        ...
        return await Task.FromResult(rowDataList);
    }
    ...
    private List<Dictionary<string, object?>> GenerateCompositeFkBatch(CompositeForeignKey cfk, int maxRowCount,
        List<TargetDataModel> existingData)
    {
        var parentTable = existingData.FirstOrDefault(t =>
        ...
        List<TargetDataModel> existingData)
    {
        ...
        foreach (var pkColName in primaryKey.PrimaryKeyColumns)
        {
            ...
            if (fkInfo != null)
            {
                ...
                Msg.Write(MessageType.Debug,
                    $"Composite PK column {pkColName} is an FK. Pulling from parent {fkInfo.ReferencedTable}.");
                values = GetPotentialFkValues(fkInfo, existingData);
            }
            else
            {
    ...
    private List<Dictionary<string, object?>> GenerateCompositeUniqueIndexBatch(
        TableModel table, List<string> indexColumnNames, int maxRowCount, List<TargetDataModel> existingData)
    {
        var keyColumnValuePools = new Dictionary<string, List<object?>>();
        long totalPossibleCombinations = 1;
    ...
    private List<object?> GenerateSingleUniqueBatch(ColumnModel column, TableModel table,
        List<RelationshipModel> relationships, int maxRowCount, List<TargetDataModel> existingData)
    {
        ...
        if (fkInfo != null)
        {
            ...
            batch = GenerateForeignKeyBatch(fkInfo, maxRowCount, true, existingData);
        }
        else
        {
        ...
        List<RelationshipModel> relationships, int maxRowCount, List<TargetDataModel> existingData)
    {
        ...
        if (fkInfo != null)
        {
            ...
            batch = GenerateForeignKeyBatch(fkInfo, maxRowCount, false, existingData);
        }
        else
        {
    ...
    private List<object?> GetPotentialFkValues(ForeignKeyInfo foreignKeyInfo, List<TargetDataModel> existingData)
    {
        var referencedTableData = existingData.FirstOrDefault(t =>
    ...
    private List<object?> GenerateForeignKeyBatch(ForeignKeyInfo foreignKeyInfo, int count, bool isUnique,
        List<TargetDataModel> existingData)
    {
        var potentialFkValues = GetPotentialFkValues(foreignKeyInfo, existingData);
        if (!potentialFkValues.Any())
        {

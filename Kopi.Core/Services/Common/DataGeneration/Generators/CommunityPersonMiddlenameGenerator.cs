...
public class CommunityPersonMiddlenameGenerator : IDataGenerator
{
    public string TypeName => "person_middlename";
    ...
    public List<object?> GenerateBatch(ColumnModel column, int count, bool isUnique = false)
    {
        ...
        if (!isUnique)
        {
            ...
            {
                values.Add(GetTruncatedMiddleName(maxLength));
            }
        ...
        while (uniqueNames.Count < targetCount && totalAttempts < maxAttempts)
        {
            var name = GetTruncatedMiddleName(maxLength);
            uniqueNames.Add(name); // Add() returns bool, but we just check Count
            totalAttempts++;
        }
        
        // 4. Handle failure (if we hit maxAttempts before targetCount)
        if (uniqueNames.Count < targetCount)
        {
            Msg.Write(MessageType.Warning, 
                $"Generator '{TypeName}' for column '{column.ColumnName}' " +
                $"could only generate {uniqueNames.Count} unique values out of requested {targetCount} after {totalAttempts} attempts.");
        }
        
        return uniqueNames.Cast<object?>().ToList();
    }
    
    private string GetTruncatedMiddleName(int maxLength)
    {

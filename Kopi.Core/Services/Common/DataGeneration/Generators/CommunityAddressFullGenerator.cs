...
public class CommunityAddressFullGenerator : IDataGenerator
{
    public string TypeName => "address_full";
    ...
    public List<object?> GenerateBatch(ColumnModel column, int count, bool isUnique = false)
    {
        ...
        if (!isUnique)
        {
            ...
            {
                values.Add(GetTruncatedFullAddress(maxLength));
            }
        ...
        {
            var address = GetTruncatedFullAddress(maxLength);
        ...
        if (uniqueAddresses.Count < targetCount)
        {
            Msg.Write(MessageType.Warning, 
                ...
                $"could only generate {uniqueAddresses.Count} unique values out of requested {count} " +
    ...
    private string GetTruncatedFullAddress(int maxLength)
    {

...
public class CommunityPersonFirstnameGenerator : IDataGenerator
{
    public string TypeName => "person_firstname";
    ...
    public List<object?> GenerateBatch(ColumnModel column, int count, bool isUnique = false)
    {
        ...
        if (!isUnique)
        {
            ...
            {
                values.Add(GetTruncatedFirstName(maxLength));
            }
        ...
        var uniqueFirstNames = new HashSet<string>();
        ...
        while (uniqueFirstNames.Count < targetCount && totalAttempts < maxAttempts)
        {
            var firstName = GetTruncatedFirstName(maxLength);
            uniqueFirstNames.Add(firstName); // Add() returns bool, but we just check Count
            totalAttempts++;
        }
        
        // 4. Handle failure (if we hit maxAttempts before targetCount)
        if (uniqueFirstNames.Count < targetCount)
        {
            Msg.Write(MessageType.Warning, 
                $"Generator '{TypeName}' for column '{column.ColumnName}' " +
                $"could only generate {uniqueFirstNames.Count} unique values out of requested {count} after {totalAttempts} attempts.");
        }
        
        return uniqueFirstNames.Cast<object?>().ToList();
    }
    
    private string GetTruncatedFirstName(int maxLength)
    {

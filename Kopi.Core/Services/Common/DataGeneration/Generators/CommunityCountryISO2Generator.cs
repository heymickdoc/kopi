...
public class CommunityCountryISO2Generator : IDataGenerator
{
    public string TypeName => "country_iso2";
    ...
    public List<object?> GenerateBatch(ColumnModel column, int count, bool isUnique = false)
    {
        ...
        if (!isUnique)
        {
            ...
            {
                values.Add(GetTruncatedCountryISO2(maxLength));
            }
        ...
        {
            var country = GetTruncatedCountryISO2(maxLength);
    ...
    private string GetTruncatedCountryISO2(int maxLength)
    {
        var countryISO2 = _faker.Address.CountryCode();
        if (countryISO2.Length > maxLength)
        {
            countryISO2 = countryISO2.Substring(0, maxLength);
        }
        return countryISO2;
    }
}
...
public class CommunityDefaultStringGenerator : IDataGenerator
{
    public string TypeName => "default_string";
    ...
    public List<object?> GenerateBatch(ColumnModel column, int count, bool isUnique = false)
    {
        ...
        if (!isUnique)
        {
            ...
            {
                values.Add(GetTruncatedString(maxLength));
            }
        ...
        while (uniqueStrings.Count < targetCount && totalAttempts < maxAttempts)
        {
            var str = GetTruncatedString(maxLength);
            uniqueStrings.Add(str); // Add() returns bool, but we just check Count
            totalAttempts++;
        }
        
        // 4. Handle failure (if we hit maxAttempts before targetCount)
        if (uniqueStrings.Count < targetCount)
        {
            Msg.Write(MessageType.Warning, 
                $"Generator '{TypeName}' for column '{column.ColumnName}' " +
                $"could only generate {uniqueStrings.Count} unique values out of requested {targetCount} after {totalAttempts} attempts.");
        }
        
        return uniqueStrings.Cast<object?>().ToList();
    }
    
    private string GetTruncatedString(int maxLength)
    {
        var str = _faker.Lorem.Sentence();

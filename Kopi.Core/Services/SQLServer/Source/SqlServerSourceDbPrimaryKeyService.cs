...
public class SqlServerSourceDbPrimaryKeyService
{
    /// <summary>
    ...
    private static async Task<List<RawSqlServerPrimaryKeyModel>> GetRawPrimaryKeyData(KopiConfig config)
    {
        const string sql = @"
            SELECT 
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawSqlServerPrimaryKeyModel>(sql);
    ...
    private static List<PrimaryKeyModel> MapRawPrimaryKeysToPrimaryKeyModel(List<RawSqlServerPrimaryKeyModel> rawPrimaryKeyData)
    {

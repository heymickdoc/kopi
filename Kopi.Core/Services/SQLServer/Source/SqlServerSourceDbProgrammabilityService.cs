...
public class SqlServerSourceDbProgrammabilityService
{
    /// <summary>
    ...
    /// <summary>
    /// Gets the programmability objects (stored procedures and functions) from the source database and returns them as a list of <see cref="RawSqlServerStoredProceduresAndFunctionsModel"/> objects.
    /// </summary>
    /// <param name="config"></param>
    /// <returns></returns>
    private static async Task<List<RawSqlServerStoredProceduresAndFunctionsModel>> GetRawStoredProceduresAndFunctions(
        KopiConfig config)
    {
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawSqlServerStoredProceduresAndFunctionsModel>(programmabilitySql);
        ...
        return new List<RawSqlServerStoredProceduresAndFunctionsModel>();
    }

    private static List<ProgrammabilityModel> TransformRawStoredProceduresAndFunctions(
        List<RawSqlServerStoredProceduresAndFunctionsModel> rawProgrammabilityItems, bool isStoredProcedure = true)
    {

...
public class SqlServerSourceDbRelationshipService
{
    ...
    /// <returns></returns>
    private static async Task<List<RawSqlServerRelationshipModel>> GetRawRelationshipData(KopiConfig config)
    {
        const string sql = @"
            SELECT 
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawSqlServerRelationshipModel>(sql);
        ...
        return new List<RawSqlServerRelationshipModel>();
    }
    
    private static List<RelationshipModel> MapRawRelationshipsToRelationshipModel(List<RawSqlServerRelationshipModel> rawRelationships)
    {

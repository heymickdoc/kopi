...
public class SourceDbTableService
{
    /// <summary>
    ...
    /// <returns></returns>
    private static async Task<List<RawDenormalizedTableModel>> GetRawTableData(KopiConfig config)
    {
        const string sql = @"
        SELECT 
            t.name AS TableName,
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawDenormalizedTableModel>(sql);
        ...
        return new List<RawDenormalizedTableModel>();
    }
    ...
    /// <returns>The list of <see cref="TableModel"/> data</returns>
    private static List<TableModel> MapRawTablesToTableModel(List<RawDenormalizedTableModel> rawTableList)
    {

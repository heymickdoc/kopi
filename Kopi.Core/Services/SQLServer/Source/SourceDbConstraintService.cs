...
/// </summary>
public class SourceDbConstraintService
{
    ...
    /// <returns></returns>
    private static List<ConstraintModel> MapRawConstraintsToConstraintModel(List<RawConstraintModel> rawConstraintData)
    {
    ...
    private static async Task<List<RawConstraintModel>> GetRawConstraintData(KopiConfig config)
    {
        const string sql = @"
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawConstraintModel>(sql);
        ...
        return new List<RawConstraintModel>();
    }
}
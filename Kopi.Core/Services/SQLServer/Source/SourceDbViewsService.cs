...
/// </summary>
public class SourceDbViewsService
{
    /// <summary>
    ...
    private static List<ViewModel> MapRawViewsToModel(List<RawViewModel> rawViews)
    {
        var views = rawViews
    ...
    /// <returns></returns>
    private static async Task<List<RawViewModel>> GetRawViewData(KopiConfig config)
    {
        const string sql = @"
            SELECT 
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawViewModel>(sql);

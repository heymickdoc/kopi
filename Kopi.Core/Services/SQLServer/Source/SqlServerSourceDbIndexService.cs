...
public class SqlServerSourceDbIndexService
{
    ...
    private static async Task<List<RawSqlServerIndexModel>> GetRawIndexData(KopiConfig config)
    {
        const string sql =
            @"SELECT 
        ...
        try
        {
            ...
            var data = await conn.QueryAsync<RawSqlServerIndexModel>(sql);
        ...
        return new List<RawSqlServerIndexModel>();
    }
    
    private static List<IndexModel> MapRawIndexesToIndexModel(List<RawSqlServerIndexModel> rawIndexes)
    {
